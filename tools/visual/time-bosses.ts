...
view.setHandlers({rotate:(x,y)=>session.rotateAt(x,y),firePressStart:fire,firePressEnd:()=>{},bulletTime:()=>session.startBulletTime(),reset:()=>{},openSettings:()=>{},closeSettings:()=>{},toggleAudio:()=>{},toggleHaptics:()=>{},selectTheme:()=>{},openLevels:()=>{},selectLevel:()=>{},canSelectLevel:()=>false,unlockAllLevels:()=>{},clearHistory:()=>{},uiChanged:()=>{},resultPrimary:()=>{},resultSecondary:()=>{},resultPreview:()=>{},resultLevels:()=>{},closePoster:()=>{},savePoster:()=>{},coinSound:()=>{}});
