...
// Emits a reviewable patch for the separate boss file. The 200 numbered
...
if(levels.length!==200)throw new Error('Expected 200 campaign levels');
const bosses=Array.from({length:levels.length/10},(_,index)=>designBoss((index+1)*10,levels[index*10+9]).level);
