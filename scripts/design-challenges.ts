...
const current=JSON.parse(readFileSync('src/levels/levels.json','utf8')) as LevelDefinition[];
if(current.length!==130)throw new Error('Historical challenge archive. Use flow-recipes.ts for the expanded campaign.');
const prefix=current.slice(0,100);
