...
export function replayBoss(design:BossDesign,fps=60){
  ...
  assert(session.state.won,`${design.level.name}: dynamic solution failed (actions=${next}, focuses=${JSON.stringify(lastTrace?.focusHits)}, hits=${JSON.stringify(lastTrace?.hits)})`);
  return session;
}
...
if(process.argv[1]?.endsWith('verify-time-bosses.ts')){
  ...
  assert.equal(createHash('sha256').update(JSON.stringify(levelsRaw)).digest('hex'),'a31c183873dde62dfc1304eab1c780f0cfb64cf3ee0584f72089588511c02819','The original 130 campaign levels changed');
