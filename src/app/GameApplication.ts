...
export class GameApplication {
    ...
    this.session.on(event=>{
      ...
      if(event.type==='impact'){
        ...
        switch(event.impact.type){
          ...
          case 'door-open': this.audio.play('mirrorHit',.35); break;
  ...
  private bindViewHandlers(){
    this.view.setHandlers({
      ...
      rotate:(x,y)=>{
        if((this.session.state.firing&&!this.session.state.timeSkill?.canOperate)||this.session.state.won||this.overlayLocked())return;
        const now=nowMs();
