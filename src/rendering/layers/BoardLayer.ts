...
export class BoardLayer extends Container {
  rebuild(level:LevelDefinition,g:BoardGeometry){
    ...
    shape.roundRect(g.ox+4,g.oy+10,g.boardW,g.boardH,24).fill({color:Theme.boardShadow,alpha:.42});
    ...
    shape.roundRect(g.ox,g.oy,g.boardW,g.boardH,24).fill(Theme.boardTop).stroke({color:Theme.white,width:2,alpha:.12});
    ...
    for(let y=0;y<level.rows;y++)for(let x=0;x<level.cols;x++){
      ...
      shape.roundRect(rx,ry,size,size,radius).fill(color).stroke({color:Theme.white,width:1,alpha:.07});
      shape.roundRect(rx+1,ry+size*.48,size-2,size*.50-1,radius*.85).fill({color:Theme.cellShade,alpha:.12});
    }
