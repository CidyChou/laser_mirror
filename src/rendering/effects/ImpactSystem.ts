...
export class ImpactSystem extends Container{
  ...
  private activate(x:number,y:number,color:number,start:number,life:number,strength:number,cross:boolean){const f=this.pool.find(v=>!v.active)??this.pool[0];f.active=true;f.start=start;f.life=life;f.color=color;f.strength=strength;f.root.visible=true;f.root.position.set(x,y);f.ring.clear().circle(0,0,7).stroke({color,width:3.6,alpha:.88});f.flash.clear().circle(0,0,7).fill({color:0xffffff,alpha:.9});f.cross.clear();if(cross)f.cross.moveTo(-14,0).lineTo(14,0).stroke({color:0xffffff,width:1.4,alpha:.5}).moveTo(0,-14).lineTo(0,14).stroke({color:0xffffff,width:1.4,alpha:.5});}
  update(now:number){for(const f of this.pool){if(!f.active)continue;const t=(now-f.start)/f.life;if(t<0){f.root.visible=false;continue;}if(t>=1){f.active=false;f.root.visible=false;continue;}f.root.visible=true;const out=1-Math.pow(1-t,2.15);f.ring.scale.set(1+out*3.15*f.strength);f.ring.alpha=(1-t)*.88;f.flash.scale.set(.65+Math.sin(Math.min(1,t*4)*Math.PI)*1.2*f.strength);f.flash.alpha=Math.max(0,1-t*3.5)*.9;f.cross.scale.set(.8+out*.9);f.cross.alpha=(1-t)*.48;}}
