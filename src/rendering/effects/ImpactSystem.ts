...
export class ImpactSystem extends Container{
  ...
  constructor(){
    ...
    for(let i=0;i<24;i++){
      ...
      const ring=new Graphics().circle(0,0,7).stroke({color:Theme.white,width:3.6,alpha:.88});
      const flash=new Graphics().circle(0,0,7).fill({color:Theme.white,alpha:.9});
      const cross=new Graphics().moveTo(-14,0).lineTo(14,0).stroke({color:Theme.white,width:1.4,alpha:.5}).moveTo(0,-14).lineTo(0,14).stroke({color:Theme.white,width:1.4,alpha:.5});
      ring.blendMode='add';cross.blendMode='add';flash.blendMode='add';
      root.addChild(flash,ring,cross);
      this.addChild(root);
      this.pool.push({root,ring,cross,flash,start:0,life:320,active:false,color:Theme.white,strength:1});
    }
  }
  triggerLaunch(x:number,y:number,now:number){this.activate(x,y,Theme.white,now,240,1.05,false);this.activate(x,y,Theme.beam,now,480,2.05,false);}
  triggerImpactEffect(e:ImpactEvent,now:number){const col=e.type==='target'||e.type==='switch'?Theme.green:e.type==='portal'?Theme.purple:e.type==='splitter'?Theme.cyan:e.type==='mirror'?Theme.white:Theme.beam;const strength=e.type==='target'?1.65:e.type==='portal'?1.4:e.type==='mirror'||e.type==='splitter'?1.28:1.12;this.activate(e.px,e.py,col,now,e.type==='target'?560:e.type==='portal'?460:360,strength,e.type==='mirror'||e.type==='splitter');if(e.type==='portal'&&e.toX!==undefined&&e.toY!==undefined)this.activate(e.toX,e.toY,col,now+50,460,1.28,false);}
  ...
  private activate(x:number,y:number,color:number,start:number,life:number,strength:number,cross:boolean){
    ...
    f.ring.tint=color;f.flash.tint=Theme.white;f.cross.visible=cross;
  }
