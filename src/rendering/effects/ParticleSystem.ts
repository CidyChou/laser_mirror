...
export class ParticleSystem {
  ...
  emit(x:number,y:number,color:number,count:number,budget:number){
    count=Math.min(count,Math.max(0,budget-this.activeParticles.length));
    for(let i=0;i<count;i++){
      const p=this.pool.pop()??new Particle({texture:this.texture}); const a=Math.random()*Math.PI*2,s=.6+Math.random()*3.1;
      p.x=x;p.y=y;p.alpha=.85;p.tint=color;const size=.35+Math.random()*.7;p.scaleX=size;p.scaleY=size;this.container.addParticle(p);
