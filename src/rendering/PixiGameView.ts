...
export class PixiGameView{
  ...
  private buildBackground(){
    ...
    this.victoryWash.rect(32,STAGE_TOP,656,STAGE_HEIGHT).fill({color:0x8fffd0,alpha:1});
  ...
  setHandlers(h:{rotate:(x:number,y:number)=>void;fire:()=>void;reset:()=>void;openSettings:()=>void;toggleAudio:()=>void;closeSettings:()=>void;resultPrimary:()=>void;resultSecondary:()=>void;coinSound:()=>void}){
  ...
  hideOverlays(){this.result.hide();this.settings.hide();this.combo.clear();this.coins.hide();this.hud.setHeartsVisible(true);}
  ...
  showWinCoins(now:number, balance:number, reward:number){
    this.hud.setHeartsVisible(false);
  ...
  get active(){return this.laser.active||this.impacts.active||this.particles.active||this.objects.active||this.toastUntil>0||this.victoryUntil>0||this.comboActive||this.resultActive||this.coinsActive;}
