...
export type GameEvent =
  | { type:'skill-start' | 'skill-end'; skill:'bullet' }
  | { type:'skill-tutorial'; skill:'bullet' }
...
export class GameSession {
  ...
  private beamAgeMs = 0;
  private spentAgeMs = 0;
  private spentStartLife = 1;
  private terminalStartLife = 1;
  state: GameState;
  ...
  private createState(index: number, hearts: number): GameState {
    ...
    return {
      timeSkill: level.timeBoss ? { adjustmentUses:level.timeBoss.adjustmentUses, bulletTimeUses:level.timeBoss.bulletTimeUses, phase:'idle', remainingMs:0, timeScale:1, canOperate:false, lifetimeRemainingMs:level.timeBoss.lifetimeMs ?? GameConfig.laser.challengeLifetimeMs, beamLife:1, tutorial:null } : undefined,
  ...
  rotateAt(x:number,y:number) {
    ...
    const spendAdjustment=()=>{
      ...
      if(this.state.firing&&time){time.adjustmentUses=Math.max(0,time.adjustmentUses-1);time.canOperate=time.adjustmentUses>0;
        if(!time.adjustmentUses){this.spentAgeMs=0;this.spentStartLife=time.beamLife;}}
    };
  ...
  update(now: number): boolean {
    ...
    const success = this.isSuccess(s.result);
    const combinedTail=(Object.keys(s.result.combinerPulses).length>0||s.result.impactEvents.some(e=>e.type==='door-open'||e.type==='portal-exit'))&&s.beamDistance<s.result.maxTravel;
  ...
  private finish(success: boolean) {
    ...
    if(s.timeSkill){s.timeSkill.phase='idle';s.timeSkill.canOperate=false;s.timeSkill.remainingMs=0;s.timeSkill.tutorial=null;}
    ...
    if (success) {
      ...
      const prefix = s.timeSkill?.beamLife===0 ? '激光已消散 · 挑战失败' : this.missPrefix();
  ...
  private startSkill(skill:'bullet'):boolean {
    ...
    t.bulletTimeUses--;t.phase='bullet';t.remainingMs=3000;t.timeScale=.2;t.canOperate=t.adjustmentUses>0;
    this.emit({type:'skill-start',skill});this.emit({type:'state'});return true;
  }
  private syncDynamic() {
    ...
    if(s.timeSkill){
      s.timeSkill.canOperate=this.launchTriggered&&s.timeSkill.adjustmentUses>0&&s.timeSkill.beamLife>0;
    }
  }
  private updateDynamic(now:number):boolean {
    ...
    while(this.realAccumulator>=10&&s.firing){
      ...
      // A single foreground clock drives both the visible tail and failure.
      // Spending the last adjustment gives the in-flight pulse time to finish.
      this.beamAgeMs+=10;
      const lifetime=s.level.timeBoss!.lifetimeMs??GameConfig.laser.challengeLifetimeMs;
      let remaining=Math.max(0,lifetime-this.beamAgeMs);
      let life=remaining/lifetime;
      if(t.adjustmentUses===0){
        this.spentAgeMs+=10;
        const spentRemaining=Math.max(0,GameConfig.laser.spentAdjustmentFadeMs-this.spentAgeMs);
        remaining=Math.min(remaining,spentRemaining);
        life=Math.min(life,this.spentStartLife*spentRemaining/GameConfig.laser.spentAdjustmentFadeMs);
      }
      if(d.finished||d.exhausted){
        if(!this.terminalMs)this.terminalStartLife=t.beamLife;
        this.terminalMs+=10;
        const stoppedRemaining=Math.max(0,GameConfig.laser.stoppedBeamFadeMs-this.terminalMs);
        remaining=Math.min(remaining,stoppedRemaining);
        life=Math.min(life,this.terminalStartLife*stoppedRemaining/GameConfig.laser.stoppedBeamFadeMs);
      }
      t.lifetimeRemainingMs=remaining;t.beamLife=life;
      if(remaining<=0){this.finish(false);break;}
      ...
      if(t.phase==='bullet'){
        ...
        if(!t.remainingMs){t.phase='idle';t.canOperate=t.adjustmentUses>0;t.timeScale=1;this.emit({type:'skill-end',skill:'bullet'});}
      }
