...
export class WebPlatform implements IPlatform{
  ...
  createAudio(){const audio=new Audio();audio.preload='auto';return audio;}
  attachCanvas(canvas:HTMLCanvasElement){const root=document.getElementById('app')!;root.innerHTML='';root.appendChild(canvas);canvas.style.width=`${this.viewport().width}px`;canvas.style.height=`${this.viewport().height}px`;}
