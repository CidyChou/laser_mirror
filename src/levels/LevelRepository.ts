...
export class LevelRepository {
  readonly normalLevels: LevelDefinition[] = (rawLevels as LevelDefinition[]).map((level, index) => ({
    ...level,
    campaign: { id: level.stageKey ?? `level:${index + 1}`, kind: 'normal', displayNumber: index + 1 },
  }));

  readonly timeBosses: LevelDefinition[] = (rawTimeBosses as LevelDefinition[]).map((level) => ({
    ...level,
    campaign: { id: level.stageKey ?? `boss:${level.chapterNo}`, kind: 'boss', displayNumber: level.chapterNo * 10 },
  }));
